...
function withoutTenantMutation(value: TenantRecord): TenantRecord {
  if (Object.prototype.hasOwnProperty.call(value, "tenant_id")) {
    const error = new Error("O tenant_id é imutável em atualizações de entidades da empresa.");
    (error as Error & { code?: string }).code = "TENANT_MUTATION_FORBIDDEN";
    throw error;
  }
  return value;
}
          ...
          get(builderTarget, builderProperty, builderReceiver) {
            ...
            if (builderProperty === "update") {
              return (values: TenantRecord, options?: unknown) =>
                applyTenantFilter(builderTarget.update(withoutTenantMutation(values), options), tenantId);
            }
