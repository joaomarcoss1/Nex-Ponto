...
export function JustificationsPage() {
  ...
  const [financialEffect, setFinancialEffect] = useState<Record<string, "deductible" | "non_deductible" | "paid_leave">>({});
    ...
    try {
      await adminFetch("/api/admin/justifications", {
        ...
        body: JSON.stringify({
          id,
          status,
          admin_observation: observation[id] || "",
          absence_type: "full_absence",
          financial_effect: financialEffect[id] || (status === "approved" ? "non_deductible" : "deductible"),
          decision_reason: observation[id] || (status === "approved" ? "Justificativa aprovada e ausência abonada." : "Justificativa rejeitada e ausência descontável."),
        })
      });
        ...
        {items.map((item) => (
          <Card key={item.id}>
            <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
              ...
              <div className="grid gap-3">
                <Field label="Efeito financeiro">
                  <Select value={financialEffect[item.id] || (item.status === "rejected" ? "deductible" : "non_deductible")} onChange={(event) => setFinancialEffect({ ...financialEffect, [item.id]: event.target.value as "deductible" | "non_deductible" | "paid_leave" })}>
                    <option value="non_deductible">Abonada — sem desconto</option>
                    <option value="deductible">Descontável</option>
                    <option value="paid_leave">Licença remunerada</option>
                  </Select>
                </Field>
