...
const url = process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.TEST_SUPABASE_URL;
const key = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.TEST_SUPABASE_SERVICE_ROLE_KEY;
if (!url || !key) {
  console.error("BLOQUEADOR: configure SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY para auditar o banco real.");
  process.exit(2);
}
...
const expectedFunctions = [
  ...
  "has_tenant_permission_v54","upsert_tenant_admin_v556","claim_background_job_v556",
  "heartbeat_background_job_v556","complete_background_job_v556","fail_background_job_v556",
];
...
console.log(`${data.schemaVersion === "5.5.6" ? "OK" : "REQUER MIGRATION"} schema ${data.schemaVersion || "não identificado"}`);
if (blocked || (data.rlsDisabled || []).length || data.schemaVersion !== "5.5.6") process.exit(1);
