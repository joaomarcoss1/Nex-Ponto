...
export async function GET(request: NextRequest) {
  try {
    ...
    const [{ data }, { data: tenantAccess }] = await Promise.all([
      context.rawSupabase
    ...
    return NextResponse.json({
      ...
      start_url: tenantAccess?.public_access_code
        ? `/?empresa=${encodeURIComponent(tenantAccess.public_access_code)}`
        : `/?tenant=${encodeURIComponent(context.tenant.slug)}`,
      scope: "/",
      display: "standalone",
