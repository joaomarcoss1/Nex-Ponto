...
export function AdminPasswordRecovery() {
  const [config, setConfig] = useState(getBrowserSupabaseConfigStatus());
  ...
  async function prepareEnvironment(force = false) {
    setConfig({ configured: false, loading: true, message: "Preparando ambiente..." });
    try {
      await loadBrowserSupabaseConfig({ force, retries: 1 });
    } catch {
      // A UI exibe a última mensagem pública segura abaixo.
    } finally {
      setConfig(getBrowserSupabaseConfigStatus());
    }
  }
  ...
  async function recover() {
    ...
    setMessage("");
    try {
      const runtimeConfig = await loadBrowserSupabaseConfig({ retries: 1 });
      setConfig(getBrowserSupabaseConfigStatus());
      const redirectTo = `${runtimeConfig.appUrl || getCachedBrowserSupabaseConfig()?.appUrl || window.location.origin}/admin/nova-senha`;
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-md content-start gap-8 pt-4">
        ...
        <section className="mt-12 rounded-[30px] bg-white p-6 shadow-[0_24px_80px_rgba(6,67,32,0.18)]">
          ...
          <div className="mt-6 grid gap-4">
            ...
            <Button size="lg" onClick={recover} disabled={loading || config.loading || !email || !config.configured} className="rounded-2xl">
              <MailCheck className="h-5 w-5" />
              {config.loading ? "Preparando ambiente..." : "Enviar instruções"}
            </Button>
            {!config.loading && !config.configured ? <Button variant="ghost" onClick={() => void prepareEnvironment(true)}>Tentar novamente</Button> : null}
          </div>
