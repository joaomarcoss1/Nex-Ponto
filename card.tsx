...
export function Card({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <section
      className={clsx(
        "animate-fade-in min-w-0 rounded-[1.35rem] border border-slate-200/80 bg-[var(--surface)] p-4 shadow-[0_18px_55px_rgba(15,23,42,0.07)] backdrop-blur transition duration-200 hover:shadow-[0_22px_70px_rgba(10,31,77,0.10)] sm:rounded-3xl sm:p-5",
        className
      )}
    >
      {children}
...
export function SectionTitle({ title, description, actions }: { title: string; description?: string; actions?: ReactNode }) {
  return (
    <div className="mb-5 flex min-w-0 flex-col gap-3 rounded-[1.35rem] border border-brand-100 bg-gradient-to-br from-white to-brand-50/80 p-4 shadow-[0_16px_45px_rgba(10,31,77,0.06)] sm:flex-row sm:items-start sm:justify-between sm:rounded-3xl sm:p-5">
      <div>
        <p className="mb-2 inline-flex rounded-full bg-brand-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-brand-800 sm:text-[11px]">NexPonto</p>
