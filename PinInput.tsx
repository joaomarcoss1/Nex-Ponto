...
export function PinInput({ value, onChange, disabled = false }: { value: string; onChange: (value: string) => void; disabled?: boolean }) {
  ...
  return (
    <div className="grid min-w-0 gap-2">
      ...
      <div className="grid min-w-0 grid-cols-4 gap-2 [grid-template-columns:repeat(4,minmax(0,1fr))]">
        {digits.map((digit, index) => (
          <div
            key={index}
            className={clsx(
              "grid aspect-square w-full min-w-0 place-items-center rounded-2xl border bg-white text-xl font-black shadow-sm transition",
