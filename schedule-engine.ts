...
export async function fetchScheduleContext(params: {
  ...
  const [schedules, occurrences, holidays] = await Promise.all([
    fetchAllRows<Record<string, unknown>>((from, to) => schedulesQuery.range(from, to)),
    fetchAllRows<Record<string, unknown>>((from, to) => occurrencesQuery.range(from, to)),
    fetchOperationalHolidays({ supabase, startDate, endDate, branchIds })
  ]);

  const publishedRules: WorkScheduleRule[] = occurrences.map((row) => {
  ...
  return {
    schedules: [...publishedRules, ...(schedules as unknown as WorkScheduleRule[])],
    holidays
  };
}
