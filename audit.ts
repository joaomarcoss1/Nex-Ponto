...
function hashIp(value: string | null | undefined) {
  ...
  const salt = process.env.AUDIT_HASH_SALT || process.env.TENANT_CONTEXT_SECRET;
  if (!salt || salt.length < 32) throw new Error("AUDIT_HASH_SALT ou TENANT_CONTEXT_SECRET inválido.");
...
export async function writeAuditLog(params: {
  ...
  const { error } = await supabase.from("audit_logs").insert({
    ...
    membership_id: context?.supportSessionId ? null : context?.membershipId || null,
    support_session_id: context?.supportSessionId || null,
