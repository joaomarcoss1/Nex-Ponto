...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  function openMoreMenu() {
    moreReturnFocusRef.current = document.activeElement instanceof HTMLElement
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      moreReturnFocusRef.current?.focus();
      moreReturnFocusRef.current = null;
    };
  }, [moreOpen]);
  ...
  useEffect(() => {
    ...
    const queueValidation = () => {
      if (!active || validationQueued) return;
    ...
    const tenantChanged = () => queueValidation();
    window.addEventListener("nexponto:tenant-changed", tenantChanged);

    void createBrowserSupabaseClient().then((supabase) => {
      if (!active) return;
      const { data } = supabase.auth.onAuthStateChange((event) => {
        ...
        if (["SIGNED_IN", "TOKEN_REFRESHED", "USER_UPDATED"].includes(event)) queueValidation();
      });
    ...
    return () => {
      active = false;
      window.removeEventListener("nexponto:tenant-changed", tenantChanged);
      unsubscribe?.();
    };
  }, [router]);
  ...
  useEffect(() => {
    ...
    async function validateSession() {
      ...
      try {
        ...
        if (
          data.session.user.app_metadata?.must_change_password
