...
export function AdminLogin({ redirectTo = "/admin", platform = false }: { redirectTo?: string; platform?: boolean } = {}) {
  ...
  async function login() {
    ...
    try {
      ...
      setError(timedOut ? "O login demorou mais que o esperado. Tente novamente." : err instanceof Error ? err.message : "Erro ao entrar.");
    } finally {
    ...
    <main className="min-h-screen overflow-hidden bg-[#f5f7fb] lg:grid lg:grid-cols-[1fr_520px]">
      ...
      <section className="relative grid min-h-screen place-items-center px-4 py-8">
        ...
        <div className="relative z-10 grid w-full max-w-md gap-5">
          <div className="w-fit rounded-2xl bg-brand-900/95 p-2 shadow-lg lg:hidden">
            <BrandMark inverse />
          </div>
            ...
            <form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); void login(); }}>
              ...
              <Field label="E-mail">
                <div className="relative">
                  ...
                  <Input name="email" autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} type="email" placeholder="admin@empresa.com" className="rounded-2xl pl-12" />
                </div>
              </Field>
              <Field label="Senha">
                <Input name="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} type="password" placeholder="••••••••" className="rounded-2xl" />
              </Field>
