...
import { ChevronLeft, ChevronRight, Check, RefreshCw, Search, X } from "lucide-react";
...
export function OvertimeReviewsPage() {
  ...
  async function load(nextPage = page, queryFilters = appliedFilters) {
    if (loadingRef.current) return;
    loadingRef.current = true;
    setLoading(true);
    const params = new URLSearchParams({ page: String(nextPage), pageSize: String(pageSize) });
    Object.entries(queryFilters).forEach(([key, value]) => value && params.set(key, value));
    try {
      const data = await adminFetch<{ reviews: OvertimeReviewDto[]; pagination: { total: number } }>(`/api/admin/overtime-reviews?${params.toString()}`);
    ...
    <AdminShell>
      ...
      <Card className="mb-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button disabled={loading} onClick={() => load(1, filters)}><Search className="h-4 w-4" />Filtrar</Button>
      ...
      <nav aria-label="Paginação das revisões" className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <p role="status" className="text-sm text-slate-600">{loading ? "Carregando..." : `Página ${page} de ${totalPages} · ${total} registros`}</p>
        <div className="flex gap-2">
          <Button variant="ghost" title="Página anterior" aria-label="Página anterior" disabled={loading || page <= 1} onClick={() => load(page - 1)}><ChevronLeft className="h-4 w-4" /></Button>
