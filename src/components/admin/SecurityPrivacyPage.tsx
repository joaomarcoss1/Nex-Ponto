...
export function SecurityPrivacyPage() {
  ...
  const [deviceDecision, setDeviceDecision] = useState<{ id: string; status: "active" | "revoked" | "blocked" } | null>(null);
  ...
  async function changeDevice() {
    ...
    try {
      ...
      setDeviceDecision(null); setDecisionReason(""); await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Erro ao atualizar dispositivo."); }
            ...
            {devices.map((device) => (
              <article key={device.id} className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {device.status !== "active" ? <Button size="sm" disabled={loading} onClick={() => { setDeviceDecision({ id: device.id, status: "active" }); setDecisionReason(""); }}><ShieldCheck className="h-4 w-4" /> Autorizar</Button> : null}
                  <Button size="sm" variant="ghost" disabled={loading} onClick={() => { setDeviceDecision({ id: device.id, status: "revoked" }); setDecisionReason(""); }}>Revogar</Button>
                  <Button size="sm" variant="danger" disabled={loading} onClick={() => { setDeviceDecision({ id: device.id, status: "blocked" }); setDecisionReason(""); }}>Bloquear</Button>
                </div>
              </article>
            ))}
