...
export function ClockPage() {
  ...
  useEffect(() => {
    publicFetch("/api/public/device", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: "Navegador do funcionário",
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 px-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-[28px] bg-white p-5 text-center shadow-[0_32px_100px_rgba(0,0,0,0.28)]">
            ...
            <div className="mt-4 grid gap-2 rounded-2xl bg-slate-50 p-3 text-left text-sm font-semibold text-slate-700">
              ...
              {lastConfirmation.nsr ? <p>NSR: {lastConfirmation.nsr}</p> : null}
              {lastConfirmation.riskLevel ? <p>Risco antifraude: {lastConfirmation.riskLevel}</p> : null}
            </div>
            {lastConfirmation.receiptUrl ? (
              <a
