...
export default function ShiftTemplatesPage() {
  ...
  async function deactivate() {
    if (!deactivateId || saving) return;
    setSaving(true); setError("");
    try {
      await adminFetch(`/api/admin/schedules/templates?id=${deactivateId}`, { method: "DELETE" });
      setDeactivateId(null);
      setMessage("Modelo desativado. As escalas publicadas mantiveram o histórico.");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Erro ao desativar modelo.");
    } finally { setSaving(false); }
  }
