...
async function processOne(request: NextRequest) {
  ...
  try {
    ...
    if (firstHeartbeat.error) throw new Error(`JOB_HEARTBEAT_FAILED:${firstHeartbeat.error.message}`);
    ...
    const secondHeartbeat = await heartbeat();
    if (secondHeartbeat.error) throw new Error(`JOB_HEARTBEAT_FAILED:${secondHeartbeat.error.message}`);
