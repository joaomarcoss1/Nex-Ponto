...
export async function GET() {
  ...
  const defaultTimezone = validateServerEnvironment(process.env, { capability: "timezone", includeAppUrl: false });
  const serviceRole = inspectServiceRoleKey(process.env.SUPABASE_SERVICE_ROLE_KEY).valid;
  ...
  const requiredFunctions = [
    ...
    "reconcile_admin_memberships_v557",
    "upsert_employee_v552",
    "register_time_entry_v4",
    "bulk_set_employee_pins_v554",
    "upsert_tenant_admin_v557",
    "count_effective_master_admins_v557",
    "rollback_tenant_admin_creation_v557",
    "restore_tenant_admin_snapshot_v557",
  ...
  if (coreConfiguration.ok) {
    try {
      ...
      supabaseAuth = !authResult.error && serviceRole;
  ...
  const checks = {
    ...
    defaultTimezone: defaultTimezone.ok,
  };
  ...
  if (!ready) {
    structuredLog("error", "readiness_failed", {
      ...
      environmentIssues: {
        ...
        defaultTimezone: defaultTimezone.issues,
      },
    });
  }
