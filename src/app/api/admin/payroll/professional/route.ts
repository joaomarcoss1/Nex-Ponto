...
import { canAccessBranch, canViewFinancialData, scopeByBranch } from "@/lib/server/branch-permissions";
...
import { fetchAllRows, fetchAllRowsByValues } from "@/lib/server/pagination";
...
const actionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("transition"), run_id: z.string().uuid(), target_status: z.enum(["attendance_pending", "calculated", "checking", "hr_approved", "financial_approved", "closed", "closed_with_exceptions", "exported", "paid", "reopened"]), reason: z.string().trim().min(5).max(500) }),
...
function isFullCalendarMonth(startDate: string, endDate: string) {
  ...
  const finalDay = new Date(Date.UTC(year, month, 0)).toISOString().slice(0, 10);
  return endDate === finalDay;
}
...
function salarySegmentsForEmployee(employee: EmployeeRow, history: SalaryHistoryRow[], startDate: string, endDate: string): SalarySegmentV51[] {
  ...
  const result: SalarySegmentV51[] = [...groups.values()];
  const activeForWholePeriod = (!employee.admission_date || employee.admission_date <= startDate)
    && (!employee.termination_date || employee.termination_date >= endDate);
  if (result.length === 1 && activeForWholePeriod && isFullCalendarMonth(startDate, endDate)) {
    result[0].fullCompetence = true;
  }
  return result;
}
...
function overtimeForEmployee(employeeId: string, rows: OvertimeRow[]): OvertimeApprovalV51[] {
  return rows.filter((row) => row.employee_id === employeeId && ["approved", "adjusted"].includes(row.status)).flatMap((row) => {
    ...
    const approvedTotal = row.approved_amount ?? null;
    const estimatedTotal = row.overtime_amount ?? null;
    const sourceValue = approvedTotal ?? estimatedTotal;
    const paymentValue = sourceValue === null
      ? null
      : row.destination === "split" && total > 0
        ? Number(sourceValue) * paymentMinutes / total
        : sourceValue;
    if (paymentMinutes > 0) result.push({ entryDate: row.entry_date, minutes: paymentMinutes, percentage: Number(row.approved_percentage ?? 50), approvedValue: approvedTotal === null ? null : paymentValue, destination: "payment" });
...
function totalAbsenceEffects(
  employee: EmployeeRow,
  salarySegments: SalarySegmentV51[],
  sessions: WorkSessionRow[],
  plannedDays: PlannedDayRow[],
  absences: AbsenceRow[],
) {
  const sessionDates = new Set(sessions.filter((session) => session.employee_id === employee.id).map((session) => session.work_date));
  const missingDates = plannedDays
    .filter((day) => day.employee_id === employee.id && day.status === "published" && !day.is_day_off && !sessionDates.has(day.work_date))
    .map((day) => day.work_date);
  const deductions: Array<{ code: string; name: string; amount: string }> = [];
  const divergences: Array<{ code: string; severity: "critical"; message: string; details: Record<string, unknown> }> = [];
  for (const date of missingDates) {
    const decision = absences.find((absence) => absence.employee_id === employee.id && absence.absence_date === date);
    if (!decision || decision.financial_effect === "pending" || decision.status === "pending") {
      divergences.push({
        code: "TOTAL_ABSENCE_DECISION_PENDING",
        severity: "critical",
        message: `Ausência total em ${date} sem decisão financeira.`,
        details: { date },
      });
      continue;
    }
    if (decision.financial_effect !== "deductible") continue;
    const salary = salarySegments.find((segment) => segment.startDate <= date && segment.endDate >= date);
    if (!salary) {
      divergences.push({
        code: "ABSENCE_SALARY_MISSING",
        severity: "critical",
        message: `Não há salário vigente para descontar a ausência de ${date}.`,
        details: { date },
      });
      continue;
    }
    const dayValue = multiplyCents(moneyToCents(salary.monthlySalary), 1n, BigInt(Math.max(1, salary.divisorDays)));
    deductions.push({ code: `ABSENCE_${date}`, name: `Falta descontável em ${date}`, amount: centsToString(dayValue) });
  }
  return { deductions, divergences, missingDates };
}
...
export async function POST(request: NextRequest) {
  ...
  const actionAuthorization = assertPayrollActionAvailable(
    auth.context.permissions,
    body.action,
    body.action === "transition" ? body.target_status as PayrollTransition : undefined,
  );
  if (!actionAuthorization.allowed) {
    return fail(actionAuthorization.reason, 403, {
      code: body.action === "transition" && body.target_status === "paid"
        ? "OFFICIAL_PAYROLL_DISABLED"
        : "PERMISSION_DENIED",
      requiredPermission: actionAuthorization.permission,
    });
  }
  ...
  let employeeQuery = scopeByBranch(auth.supabase.from("employees").select("id,branch_id,full_name,monthly_salary,admission_date,termination_date").eq("active", true).order("id"), auth.context, "branch_id");
  if (run.branch_id) employeeQuery = employeeQuery.eq("branch_id", run.branch_id);
  let employees: EmployeeRow[];
  try {
    employees = await fetchAllRows<EmployeeRow>((from, to) => employeeQuery.range(from, to), { maxRows: 100_000 });
  } catch (cause) {
    return fail("Erro ao carregar todos os funcionários.", 500, cause instanceof Error ? cause.message : "PAGINATION_FAILED");
  }
  ...
  try {
    [salaryRows, contractRows, overtimeRows, hourBankRows, sessions, legalTables, plannedDays, absences] = await Promise.all([
      fetchAllRowsByValues<SalaryHistoryRow>(employeeIds, (ids, from, to) => auth.supabase.from("employee_salary_history").select("id,employee_id,monthly_salary,valid_from,effective_from,valid_until").in("employee_id", ids).lte("valid_from", period.end_date).or(`valid_until.is.null,valid_until.gte.${period.start_date}`).order("id").range(from, to)),
      fetchAllRowsByValues<ContractRow>(employeeIds, (ids, from, to) => auth.supabase.from("employee_contract_rules").select("id,employee_id,salary_hour_divisor,salary_day_divisor,night_shift_rule,effective_from,effective_until,payroll_rule_sets(status)").in("employee_id", ids).lte("effective_from", period.end_date).or(`effective_until.is.null,effective_until.gte.${period.start_date}`).order("id").range(from, to)),
      fetchAllRowsByValues<OvertimeRow>(employeeIds, (ids, from, to) => auth.supabase.from("overtime_reviews").select("id,employee_id,entry_date,status,approved_overtime_minutes,approved_percentage,approved_amount,overtime_amount,destination,payment_minutes,bank_minutes").in("employee_id", ids).gte("entry_date", period.start_date).lte("entry_date", period.end_date).order("id").range(from, to)),
      fetchAllRowsByValues<HourBankRow>(employeeIds, (ids, from, to) => auth.supabase.from("hour_bank_movements").select("id,employee_id,movement_type,minutes,status,movement_date,expires_on,reversal_of").in("employee_id", ids).lte("movement_date", period.end_date).order("id").range(from, to)),
      fetchAllRowsByValues<WorkSessionRow>(employeeIds, (ids, from, to) => auth.supabase.from("work_sessions").select("id,employee_id,branch_id,work_date,schedule_snapshot,schedule_snapshot_checksum,contract_snapshot,status").in("employee_id", ids).gte("work_date", period.start_date).lte("work_date", period.end_date).order("id").range(from, to)),
      fetchAllRows<LegalTableRow>((from, to) => auth.supabase.from("payroll_legal_tables").select("id,table_type,status,effective_from,effective_until,version").eq("status", "homologated").lte("effective_from", period.end_date).or(`effective_until.is.null,effective_until.gte.${period.start_date}`).order("id").range(from, to), { maxRows: 10_000 }),
      fetchAllRowsByValues<PlannedDayRow>(employeeIds, (ids, from, to) => auth.supabase.from("schedule_occurrences").select("employee_id,work_date,is_day_off,status").in("employee_id", ids).gte("work_date", period.start_date).lte("work_date", period.end_date).order("employee_id").order("work_date").range(from, to)),
      fetchAllRowsByValues<AbsenceRow>(employeeIds, (ids, from, to) => auth.supabase.from("absence_justifications").select("employee_id,absence_date,status,financial_effect,absence_type").in("employee_id", ids).gte("absence_date", period.start_date).lte("absence_date", period.end_date).order("employee_id").order("absence_date").range(from, to)),
    ]);
  } catch (cause) {
    return fail("Erro ao montar todos os dados históricos do cálculo.", 500, cause instanceof Error ? cause.message : "PAGINATION_FAILED");
  }

  const sessionIds = sessions.map((session) => session.id);
  let timeEntries: TimeEntryRow[];
  try {
    timeEntries = await fetchAllRowsByValues<TimeEntryRow>(sessionIds, (ids, from, to) => auth.supabase.from("time_entries").select("id,employee_id,work_session_id,action,entry_timestamp,status,late_minutes,early_leave_minutes").in("work_session_id", ids).order("id").range(from, to), { maxRows: 1_000_000 });
  } catch (cause) {
    return fail("Erro ao carregar todas as marcações vinculadas às jornadas.", 500, cause instanceof Error ? cause.message : "PAGINATION_FAILED");
  }
  ...
  let brackets: LegalBracketRow[];
  try {
    brackets = await fetchAllRowsByValues<LegalBracketRow>(legalIds.length ? legalIds : [emptyId], (ids, from, to) => auth.rawSupabase.from("payroll_legal_brackets").select("legal_table_id,sequence,lower_bound,upper_bound,rate,deduction").in("legal_table_id", ids).order("legal_table_id").order("sequence").range(from, to), { maxRows: 50_000 });
  } catch (cause) {
    return fail("Erro ao carregar todas as faixas legais.", 500, cause instanceof Error ? cause.message : "PAGINATION_FAILED");
  }
