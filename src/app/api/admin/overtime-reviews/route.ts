...
export async function GET(request: NextRequest) {
  const auth = await requireAdmin(request, { all: ["overtime.review"] });
  ...
  let query = scopeByBranch(auth.supabase
    ...
    .select("*, employees(full_name, role), branches:branches!overtime_reviews_branch_id_fkey(name)", { count: "exact" })
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const typedEmployees = await fetchAllRows<Employee>((from, to) => employeesQuery.range(from, to));
    ...
    const entriesQuery = auth.supabase.from("time_entries").select("*").in("employee_id", employeeIds).gte("entry_date", startDate).lte("entry_date", endDate);
    const existingQuery = auth.supabase.from("overtime_reviews").select("employee_id,entry_date").in("employee_id", employeeIds).gte("entry_date", startDate).lte("entry_date", endDate);
    const [typedEntries, typedExisting] = await Promise.all([
      fetchAllRows<TimeEntry>((from, to) => entriesQuery.range(from, to)),
      fetchAllRows<ExistingOvertimeKey>((from, to) => existingQuery.range(from, to)),
    ]);
...
export async function PATCH(request: NextRequest) {
  const auth = await requireAdmin(request, { all: ["overtime.review"] });
  if ("error" in auth) return auth.error;
  const parsed = overtimeApprovalSchema.safeParse(await readJson<unknown>(request));
  ...
  const idempotencyKey = body.idempotency_key || `${auth.context.tenantId}:overtime:${body.id}`;
  const { data, error } = await auth.rawSupabase.rpc("approve_overtime_v51", {
    ...
    p_approved_amount: body.status === "rejected"
      ? 0
      : body.approved_amount === null
        ? null
        : normalizeMoney(body.approved_amount),
