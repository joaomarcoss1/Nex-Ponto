...
export async function GET(request: NextRequest) {
  ...
  try {
    const movements = await fetchAllRows<any>((from, to) => query.range(from, to), { maxRows: 100_000 });
    return ok({ movements, recordsProcessed: movements.length });
  } catch (cause) {
    return fail("Erro ao listar banco de horas.", 500, cause instanceof Error ? cause.message : "PAGINATION_FAILED");
  }
}
