...
export async function GET(request: NextRequest) {
  ...
  const { data: tenantAccess, error: tenantAccessError } = await auth.rawSupabase
    .from("tenants")
    .select("public_access_code")
    .eq("id", auth.context.tenantId)
    .maybeSingle();
  if (tenantAccessError) {
    return fail("Não foi possível carregar os dados da empresa.", 500, {
      code: "DATABASE_ERROR",
      requestId: request.headers.get("x-request-id") || undefined,
      technicalMessage: tenantAccessError.message,
    });
  }
