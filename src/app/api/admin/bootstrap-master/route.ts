...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      return fail("Não foi possível concluir a criação atômica da empresa e do proprietário. O usuário de autenticação foi preservado para uma nova tentativa.", 500, {
        code: "DATABASE_ERROR",
        authUserPreserved: Boolean(createdAuthUserId),
    ...
    return fail("Erro inesperado na configuração inicial. Qualquer usuário de autenticação criado foi preservado para uma nova tentativa.", 500, {
      code: "INTERNAL_ERROR",
      authUserPreserved: Boolean(createdAuthUserId),
      technicalMessage: error instanceof Error ? error.message : "unknown",
    });
  }
}
