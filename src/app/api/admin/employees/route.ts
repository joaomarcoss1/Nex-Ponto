...
export async function GET(request: NextRequest) {
  ...
  if (paginated) {
    ...
    if (result.error) return fail("Erro ao listar funcionários.", 500, result.error.message);
    data = (result.data || []) as EmployeeRow[];
    count = result.count || 0;
  } else {
    try {
      data = await fetchAllRows<EmployeeRow>((from, to) => query.range(from, to));
    } catch (cause) {
      return fail("Erro ao listar funcionários.", 500, cause instanceof Error ? cause.message : "PAGINATION_FAILED");
    }
  }
  const financialAllowed = canViewFinancialData(auth.context);
  const employees = data.map(publicEmployee).map((employee) => financialAllowed ? employee : maskSensitiveEmployeeFields(employee || {}, auth.context));
...
async function saveEmployee(request: NextRequest, mode: "create" | "update") {
  ...
  if (mode === "create") {
    ...
    if (!z.string().uuid().safeParse(idempotencyKey).success) {
      return fail("A identificação segura desta tentativa de cadastro está ausente.", 400, { code: "VALIDATION_FAILED" });
    }
    try {
  ...
  const { data, error } = await auth.rawSupabase.rpc("upsert_employee_v552", {
    ...
    p_request_id: mode === "create" ? request.headers.get("idempotency-key") : null,
  });
  if (error) {
    const closed = error.message.includes("CLOSED_PERIOD");
    const duplicate = error.code === "23505" || /duplicate|unique/i.test(error.message);
    return fail(
      closed ? "A vigência informada está em uma competência fechada." : duplicate ? "Já existe um funcionário com esta matrícula ou documento nesta empresa." : "Erro ao salvar funcionário de forma transacional.",
      closed || duplicate ? 409 : 422,
      { code: closed || duplicate ? "CONFLICT" : "VALIDATION_FAILED", technicalMessage: error.message },
    );
  }
