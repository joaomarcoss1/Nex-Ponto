...
export async function POST(request: NextRequest) {
  ...
  const periodResult = await auth.supabase.from("payroll_periods").select("title,start_date,end_date,branch_id").eq("id", run.payroll_period_id).maybeSingle();
  if (periodResult.error || !periodResult.data) return fail("Erro ao montar a exportação profissional.", 500, periodResult.error?.message);
  let employees: ProfessionalReportEmployeeV51[];
  let rubrics: ProfessionalReportRubricV51[];
  let divergences: ProfessionalReportDivergenceV51[];
  let approvals: ProfessionalReportApprovalV51[];
  try {
    let employeeQuery = auth.supabase.from("employees").select("id,full_name,registration_code,branch_id,role").eq("active", true).order("id");
    if (run.branch_id) employeeQuery = employeeQuery.eq("branch_id", run.branch_id);
    [employees, rubrics, divergences, approvals] = await Promise.all([
      fetchAllRows<ProfessionalReportEmployeeV51>((from, to) => employeeQuery.range(from, to), { maxRows: 100_000 }),
      fetchAllRows<ProfessionalReportRubricV51>((from, to) => auth.supabase.from("payroll_item_rubrics").select("employee_id,rubric_code,rubric_name,rubric_type,final_value,formula_snapshot").eq("calculation_run_id", run.id).order("employee_id").order("sequence").range(from, to), { maxRows: 1_000_000 }),
      fetchAllRows<ProfessionalReportDivergenceV51>((from, to) => auth.supabase.from("payroll_divergences").select("employee_id,code,severity,message,status").eq("calculation_run_id", run.id).order("id").range(from, to), { maxRows: 500_000 }),
      fetchAllRows<ProfessionalReportApprovalV51>((from, to) => auth.supabase.from("payroll_approvals").select("approval_stage,decision,reason,approved_at").eq("calculation_run_id", run.id).order("id").range(from, to), { maxRows: 100_000 }),
    ]);
  } catch (cause) {
    return fail("Erro ao paginar todos os dados da exportação profissional.", 500, cause instanceof Error ? cause.message : "PAGINATION_FAILED");
  }
  ...
  try {
    const buffer = input.format === "pdf"
      ...
      : await createProfessionalPayrollXlsxV51({ ...params, approvals });
    ...
    await writeAuditLog({ supabase: auth.supabase, context: auth.context, action: "export_professional_payroll_v51", entity: "payroll_calculation_runs", entityId: run.id, newData: { format: input.format, checksum, export_id: exportRow.id, recordsProcessed: rubrics.length + divergences.length } });
