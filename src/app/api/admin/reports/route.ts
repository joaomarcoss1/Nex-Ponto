...
function buildTable(type: string, data: any[], footer: string, meta: string[] = [], format: string = "json"): ExportTable {
  ...
  return {
    title: type === "payroll" ? "Pré-folha e Conferência de Jornada" : "Relatório de Pré-folha e Jornada",
...
async function loadLunchReportRows(auth: Awaited<ReturnType<typeof requireAdmin>> & any, params: URLSearchParams) {
  ...
  return [...groups.values()].map((entries) => {
    ...
    return {
      ...
      lunch_return_delay_minutes: end?.schedule_compliance_status === "lunch_late"
        ? Math.max(0, Number(end?.lunch_variation_minutes ?? 0))
        : 0,
      lunch_over_minutes: end?.schedule_compliance_status === "lunch_long"
        ? Math.max(0, Number(end?.lunch_variation_minutes ?? 0))
        : 0,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (type === "absences") {
      ...
      data = await fetchReportRows(query);
    } else if (type === "overtime") {
      let query = scopeByBranch(auth.supabase.from("overtime_reviews").select("*, employees(full_name, role), branches:branches!overtime_reviews_branch_id_fkey(name)").order("entry_date", { ascending: false }).order("id"), auth.context, "branch_id");
      if (params.get("branchId")) query = query.eq("branch_id", params.get("branchId"));
      if (params.get("employeeId")) query = query.eq("employee_id", params.get("employeeId"));
      if (params.get("status")) query = query.eq("status", params.get("status"));
      if (params.get("startDate")) query = query.gte("entry_date", params.get("startDate"));
      ...
      let query = scopeByBranch(auth.supabase.from("absence_justifications").select("*, employees(full_name, role), branches:branches!absence_justifications_branch_id_fkey(name)").order("absence_date", { ascending: false }).order("id"), auth.context, "branch_id");
      if (params.get("branchId")) query = query.eq("branch_id", params.get("branchId"));
      if (params.get("employeeId")) query = query.eq("employee_id", params.get("employeeId"));
      if (params.get("status")) query = query.eq("status", params.get("status"));
      ...
      let query = scopeByBranch(auth.supabase.from("time_entries").select("*, employees(full_name, role), branches:branches!time_entries_branch_id_fkey(name)").order("entry_timestamp", { ascending: true }).order("id"), auth.context, "branch_id");
      if (params.get("branchId")) query = query.eq("branch_id", params.get("branchId"));
      if (params.get("employeeId")) query = query.eq("employee_id", params.get("employeeId"));
      if (params.get("startDate")) query = query.gte("entry_date", params.get("startDate"));
      if (params.get("endDate")) query = query.lte("entry_date", params.get("endDate"));
    ...
    if (format === "pdf") {
      await writeAuditLog({ supabase: auth.supabase, context: auth.context, action: "export_pdf", entity: "reports", newData: { type, filters: Object.fromEntries(params.entries()), rows: data.length, financial: financialAllowed }, headers: request.headers });
    ...
    if (format === "xlsx") {
      await writeAuditLog({ supabase: auth.supabase, context: auth.context, action: "export_xlsx", entity: "reports", newData: { type, filters: Object.fromEntries(params.entries()), rows: data.length, financial: financialAllowed }, headers: request.headers });
    ...
    return ok({ rows: data, table, recordsProcessed: data.length });
  } catch (error) {
    if (error instanceof PaginationLimitError) {
      return fail("O relatório excede 100.000 registros para processamento síncrono. Reduza o período ou aplique filtros; nenhum registro foi truncado.", 413, { code: "REPORT_SYNC_LIMIT_EXCEEDED" });
    }
    if (error instanceof PaginationTimeoutError) {
      return fail("A consulta do relatório excedeu o tempo operacional. Tente novamente com um período menor.", 504, { code: "REPORT_QUERY_TIMEOUT" });
    }
