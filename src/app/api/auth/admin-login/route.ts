...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const protectedPasswordChange = Boolean(data.user.app_metadata?.must_change_password);
    const legacyPasswordChange = Boolean(data.user.user_metadata?.must_change_password);
    if (!protectedPasswordChange && legacyPasswordChange) {
      ...
      if (metadataError) {
        structuredLog("error", "admin_login_metadata_migration_failed", {
          requestId,
          userId: data.user.id,
        });
        return noStore(fail("Não foi possível validar a troca obrigatória de senha.", 503, {
    ...
    return noStore(ok({
      ...
      user: {
        ...
        mustChangePassword: Boolean(
          protectedPasswordChange || legacyPasswordChange,
        ),
      },
    }));
  } catch (cause) {
