...
export function fail(message: string, status = 400, details?: unknown) {
  ...
  const suppliedRequestId = details && typeof details === "object" && "requestId" in details
    ? String((details as { requestId?: unknown }).requestId || "").trim()
    : "";
  const requestId = /^[a-zA-Z0-9._:-]{8,180}$/.test(suppliedRequestId)
    ? suppliedRequestId
    : randomUUID();
