...
function collectCapabilityIssues(
  ...
  if (["timezone", "build", "full"].includes(capability)) {
    const timezone = env.DEFAULT_TIMEZONE?.trim();
    if (!timezone) issues.push(issue("DEFAULT_TIMEZONE", "missing"));
    else if (!isValidTimezone(timezone)) issues.push(issue("DEFAULT_TIMEZONE", "invalid_timezone"));
  }
