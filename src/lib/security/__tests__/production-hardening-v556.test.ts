...
describe("production hardening v5.5.6", () => {
  it("admin lifecycle uses the transactional admin/membership RPC", () => {
    ...
    expect(route).toMatch(/rpc\("upsert_tenant_admin_v55[67]"/);
  ...
  it("jobs, readiness, PWA and mobile shell use v5.5.6 contracts", () => {
    ...
    expect(readiness).toMatch(/"upsert_tenant_admin_v55[67]"/);
    expect(readiness).toMatch(/"5\.5\.[67]"/);
    expect(source("public/sw.js")).toMatch(/nexponto-v5\.5\.[67]-static/);
