...
assert(["5.5.6", "5.5.7", "5.5.8"].includes(packageJson.version), "package.json deve declarar v5.5.6 ou release compatível posterior.");
...
assert(/upsert_tenant_admin_v55[67]/.test(adminsRoute), "API de administradores deve usar RPC transacional v556+.");
...
assert(/nexponto-v5\.5\.[678]-static/.test(sw), "service worker deve usar cache v5.5.6 ou posterior.");
assert(/NexPonto v5\.5\.[678] quality gate/.test(ci) && /structure:v56/.test(ci), "CI deve preservar o gate v5.5.6.");
