...
const requiredFiles = [
  ...
  "src/app/api/auth/admin-password-change/route.ts",
  ...
  "supabase/migrations/060_nexponto_v557_final_production_validation.sql",
  "supabase/migrations/061_nexponto_v558_auth_acl_hardening.sql",
  ...
  "scripts/sql/precheck-v557.sql",
  "scripts/sql/postcheck-v557.sql",
  "scripts/sql/postcheck-v558.sql",
  "scripts/verify-v57-production.mjs",
  "scripts/verify-v58-production.mjs",
  "docs/production/VERCEL_DEPLOYMENT.md",
...
if (!migrations.some((name) => name.startsWith("060"))) failures.push("migration de validação final v5.5.7 ausente");
if (!migrations.some((name) => name.startsWith("061"))) failures.push("migration de hardening Auth/ACL v5.5.8 ausente");

const vercelConfig = read("vercel.json");
if (!vercelConfig.includes("/api/internal/jobs/process")) failures.push("executor de jobs não está agendado no vercel.json");
...
if (liveConfigured) {
  try {
    ...
    const [authResult, adminResult, versionResult, storageResult, structureResult] = await Promise.all([
      ...
      supabase.from("admin_users").select("id,auth_user_id,tenant_id,role,active,allowed_branch_ids").ilike("email", masterEmail),
      supabase.from("nexponto_schema_versions").select("version").eq("version", "5.5.8").maybeSingle(),
    ...
    if (admin) {
      const { data: membership, error } = await supabase.from("tenant_memberships")
        .select("id,active,tenant_id,role,branch_ids,tenants!inner(status)")
      ...
      if (membership && (membership.role !== admin.role || JSON.stringify([...(membership.branch_ids || [])].sort()) !== JSON.stringify([...(admin.allowed_branch_ids || [])].sort()))) {
        failures.push("role ou filiais do Master divergem entre admin_users e tenant_memberships");
      }
    }
    if (versionResult.error || !versionResult.data) failures.push("migration v5.5.8 não aplicada no banco");
    ...
    for (const name of ["has_tenant_permission", "has_tenant_permission_v54", "reconcile_admin_memberships_v557", "upsert_employee_v552", "register_time_entry_v4", "bulk_set_employee_pins_v554", "upsert_tenant_admin_v557", "count_effective_master_admins_v557", "rollback_tenant_admin_creation_v557", "restore_tenant_admin_snapshot_v557", "claim_background_job_v556", "heartbeat_background_job_v556", "complete_background_job_v556", "fail_background_job_v556"]) {
      if (!functions.has(name)) failures.push(`RPC crítica ausente: ${name}`);
    }
    const { count: mfaEnabled, error: mfaError } = await supabase
      .from("platform_superadmins")
      .select("id", { count: "exact", head: true })
      .eq("mfa_required", true);
    if (mfaError || Number(mfaEnabled || 0) > 0) failures.push("MFA ainda está marcado como obrigatório para um superadmin");
  } catch {
