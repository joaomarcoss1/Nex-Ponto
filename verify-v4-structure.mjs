...
includes("src/app/api/internal/jobs/holidays/route.ts", /isAuthorizedInternalJob/, "job interno deve usar autenticação dedicada");
includes("src/lib/server/internal-job-auth.ts", /INTERNAL_JOBS_SECRET/, "autenticação de jobs deve aceitar segredo interno dedicado");

const serviceWorker = read("public/sw.js");
assert(!new RegExp(["bri", "lho"].join(""), "i").test(serviceWorker), "Service worker contém marca anterior.");
assert(/nexponto-v(4|5\.[1-9])/i.test(serviceWorker), "Service worker deve usar cache versionado do NexPonto.");

const packageJson = JSON.parse(read("package.json"));
assert(/^([4-9]|\d{2,})\.\d+\.\d+$/.test(packageJson.version), "package.json deve declarar uma versão NexPonto homologável.");
