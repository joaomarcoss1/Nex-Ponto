...
export function AdminShell({ children }: { children: React.ReactNode }) {
    ...
    async function validateSession() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 409 && payload?.details?.code === "TENANT_SELECTION_REQUIRED") {
            setAuthState("redirecting");
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(18,104,243,0.10),transparent_340px),linear-gradient(180deg,#f7faf7,#f8fafc)]">
