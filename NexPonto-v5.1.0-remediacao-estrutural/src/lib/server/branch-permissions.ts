...
const ALL_BRANCH_ROLES = new Set(["tenant_owner", "tenant_admin", "rh_admin", "finance_admin", "regional_manager", "master_admin", "admin", "admin_geral", "rh_financeiro"]);
const FINANCIAL_ROLES = new Set(["tenant_owner", "finance_admin", "master_admin", "rh_financeiro"]);
const BRANCH_MANAGEMENT_ROLES = new Set(["tenant_owner", "tenant_admin", "regional_manager", "master_admin", "admin", "admin_geral"]);
const PAYROLL_ROLES = new Set(["tenant_owner", "finance_admin", "master_admin", "rh_financeiro"]);
...
export function canManagePayroll(context: AdminContext) {
  return PAYROLL_ROLES.has(context.role) || hasPermission(context, "payroll.write") || hasPermission(context, "financial.write");
}

export function canManageHourBank(context: AdminContext) {
  return canManagePayroll(context) || hasPermission(context, "hour_bank.manage");
}
