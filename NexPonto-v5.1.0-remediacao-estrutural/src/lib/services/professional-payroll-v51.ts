...
export type SalarySegmentV51 = { startDate: string; endDate: string; monthlySalary: string | number; eligibleDays: number; divisorDays: number };
...
export function calculateProfessionalPayrollV51(input: ProfessionalPayrollInputV51) {
  ...
  for (const segment of input.salarySegments) {
    ...
    const value = multiplyCents(monthly, BigInt(segment.eligibleDays), BigInt(segment.divisorDays));
    baseSalary += value;
