...
export async function GET(request: NextRequest) {
  try {
    ...
    const [openSessionRes, todayEntriesRes, occurrencesRes, fallbackSchedulesRes, movementsRes, requestsRes, notificationsRes] = await Promise.all([
      ...
      supabase.from("hour_bank_movements").select("minutes,status,expires_on").eq("employee_id", employee.id).in("status", ["approved", "reversed"]),
    ...
    const balanceMinutes = (movementsRes.data || []).reduce((total, movement) => total + Number(movement.minutes || 0), 0);
    return ok({
