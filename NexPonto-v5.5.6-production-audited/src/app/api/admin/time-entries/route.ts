...
export async function GET(request: NextRequest) {
  ...
  let query = scopeByBranch(auth.supabase
    .from("time_entries")
    .select("*, employees(full_name, role), branches:branches!time_entries_branch_id_fkey(name, timezone)", { count: "exact" })
  ...
  return ok({ entries: data || [], pagination: { page, pageSize, total: count || 0 } });
}
