...
import { requireReceiptConfig } from "@/lib/config/environment";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const defaultTimezone = branch?.timezone || process.env.DEFAULT_TIMEZONE || "America/Fortaleza";
    ...
    const timezone = branch.timezone || process.env.DEFAULT_TIMEZONE || "America/Fortaleza";
    ...
    try {
      ...
      receiptWarning = "Comprovante temporariamente indisponível. O ponto foi registrado e poderá ser recuperado depois.";
      structuredLog("error", "clock_receipt_generation_failed_after_commit", {
        requestId: request.headers.get("x-request-id"),
        tenantId: tenant.id,
        timeEntryId: confirmedEntry.id,
    ...
    return fail("Não foi possível registrar o ponto agora. Nenhuma confirmação foi emitida; tente novamente.", 503, error instanceof Error ? error.message : error);
  }
}
