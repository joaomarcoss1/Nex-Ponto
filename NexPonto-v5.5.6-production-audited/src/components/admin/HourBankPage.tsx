...
export function HourBankPage() {
  ...
  const [reversalMovement, setReversalMovement] = useState<Movement | null>(null);
  ...
  async function reverse() {
    if (!reversalMovement || reversalReason.trim().length < 5 || saving) return;
    setSaving(true); setError("");
    try {
      await adminFetch("/api/admin/hour-bank", { method: "DELETE", body: JSON.stringify({ id: reversalMovement.id, reason: reversalReason.trim() }) });
      setMessage("Estorno registrado como novo movimento. O histórico original foi preservado.");
      setReversalMovement(null); setReversalReason(""); await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Erro ao registrar estorno."); }
    finally { setSaving(false); }
  }
