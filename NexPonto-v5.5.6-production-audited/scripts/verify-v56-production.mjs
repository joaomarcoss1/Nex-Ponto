...
const pointReviews = read("src/app/api/admin/point-reviews/route.ts");
const overtimeReviews = read("src/app/api/admin/overtime-reviews/route.ts");
...
assert(packageJson.dependencies.next === "15.5.25", "Next.js deve estar em versão corrigida para produção.");
assert(packageJson.dependencies.sharp === "0.35.4", "Sharp deve estar em versão corrigida para produção.");
assert(packageJson.devDependencies["eslint-config-next"] === "15.5.25", "eslint-config-next deve acompanhar a versão do Next.js.");
assert(packageJson.devDependencies.vitest === "4.1.11", "Vitest deve estar em versão corrigida para a cadeia de testes.");
...
assert(!/user_metadata:\s*\{[^}]*\brole\b/.test(adminsRoute), "Auth user_metadata não pode gravar role administrativo paralelo ao RBAC canônico.");
...
assert(!/\.limit\(600\)/.test(pointReviews), "point-reviews não pode manter teto fixo de 600 registros.");
assert(/pagination/.test(pointReviews) && /\.range\(/.test(pointReviews), "point-reviews deve ser paginado.");
assert(!/\.limit\(600\)/.test(overtimeReviews), "overtime-reviews não pode manter teto fixo de 600 registros.");
assert(/pagination/.test(overtimeReviews) && /\.range\(/.test(overtimeReviews), "overtime-reviews deve ser paginado.");
